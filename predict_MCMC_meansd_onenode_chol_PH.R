		...
		for(i in 2:(dim(predict.grid)[1])){
			...
			if(sum(abs(predcov.values.scaled)>10)==0){
				...
				for(dp in 1:dim(depth.pred)[1]){
					...
					pred.mean <- mean(pred.post.draw)
					pred.sd <- apply(log(pred.post.draw), 1, sd)
					write.table(cbind(t(as.matrix(predict.grid[i, ]*1000)), pred.mean), paste(line.split[[1]][1], "depth", depth.pred[dp,1], "PH.mean.txt", sep="."), append=TRUE, quote=FALSE, row.names=FALSE, col.names=FALSE)
					write.table(cbind(t(as.matrix(predict.grid[i, ]*1000)), pred.sd), paste(line.split[[1]][1], "depth", depth.pred[dp,1], "PH.sd.txt", sep="."), append=TRUE,quote=FALSE, row.names=FALSE, col.names=FALSE)
				}
			}else{
				for(dp in 1:dim(depth.pred)[1]){
					pred.mean <- NA
			...
			write.table(cbind(t(as.matrix(predict.grid[i, ]*1000)), as.matrix(proc.time()-ptm)[3]), paste(line.split[[1]][1], "PH.processtime.txt", sep="."), append=TRUE,quote=FALSE, row.names=FALSE, col.names=FALSE)	
			if((proc.time()-startt)[3]>heartbeat_threshold){
				write((proc.time()-startt)[3], stderr())
			}
		}
